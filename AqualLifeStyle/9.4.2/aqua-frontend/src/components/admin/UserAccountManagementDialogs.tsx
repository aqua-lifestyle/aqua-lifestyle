...
export const ResetUserPasswordDialog = ({ onUpdated, user }: DialogProps) => {
  const [open, setOpen] = useState(false); const [submitting, setSubmitting] = useState(false); const [error, setError] = useState<string>();
  const close = () => { setOpen(false); setError(undefined); };
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const reason = adminAuditJustificationSchema.safeParse(data.get("justification")); if (!reason.success) { setError("Provide a clear reason for the password reset."); return; } setSubmitting(true); try { await httpClient.post("/api/services/app/AdminUser/ResetPassword", { id: user.id, justification: reason.data }); await onUpdated(); close(); } catch (requestError) { setError(getRequestErrorMessage(requestError, "The password reset email could not be sent.")); } finally { setSubmitting(false); } };
  return <><Button onClick={() => setOpen(true)} size="sm" variant="outline">Send password reset email</Button><Dialog onClose={close} open={open} title="Send password reset email"><form className="flex flex-col gap-4" onSubmit={submit}><p className="text-sm text-muted-foreground">The user will receive a one-time link and choose a new private password. Administrators cannot see or set it.</p><TextAreaField label="Reason for reset" maxLength={500} name="justification" required rows={3} />{error ? <StatusMessage tone="error">{error}</StatusMessage> : null}<div className="flex justify-end gap-3"><Button onClick={close} variant="ghost">Cancel</Button><Button isLoading={submitting} type="submit">Send reset email</Button></div></form></Dialog></>;
};
