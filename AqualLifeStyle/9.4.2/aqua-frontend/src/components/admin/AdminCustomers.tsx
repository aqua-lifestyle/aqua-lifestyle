...
type AdminCustomer = { contactNumber: string | null; creationTime: string; email: string; firstName: string; homeAddress: string | null; id: number; isActive: boolean; lastName: string; membershipId: number | null; membershipName: string | null; name: string; tenantId: number; userId: number };
...
export const AdminCustomers = () => {
  ...
  const columns = [
    { header: "Customer", key: "name", sortable: true, render: (customer: AdminCustomer) => <div className="flex items-center gap-3"><Avatar fallback={customer.name} size="sm" /><div><p className="font-semibold">{customer.name}</p><p className="text-xs text-muted-foreground">{customer.email}</p><p className="text-xs text-muted-foreground">{customer.contactNumber ?? "No contact number"}</p></div></div> },
