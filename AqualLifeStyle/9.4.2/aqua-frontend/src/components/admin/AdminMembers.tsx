...
export const AdminMembers = () => {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      setError(getRequestErrorMessage(requestError, "Members could not be loaded."));
    } finally {
      setLoading(false);
    }
  }, [canView]);
  ...
  const suspendMember = async (member: AdminMember, justification: string) => {
    ...
    toast({ message: `${member.firstName} ${member.lastName} has been suspended.`, title: "Member suspended", type: "success" });
    await loadMembers();
  };
  const refreshAfterMutation = async (message: string) => {
    toast({ message, title: "Member updated", type: "success" });
    await loadMembers();
  };
  ...
  const columns = [
    { header: "Member", key: "firstName", sortable: true, render: (member: AdminMember) => <div className="flex items-center gap-3"><Avatar fallback={`${member.firstName} ${member.lastName}`} size="sm" /><div><p className="font-semibold">{member.firstName} {member.lastName}</p><p className="text-xs text-muted-foreground">{member.email}</p></div></div> },
    { header: "Tenant", key: "tenantId", sortable: true, render: (member: AdminMember) => `#${member.tenantId}` },
    ...
    { header: "Actions", key: "actions", render: (member: AdminMember) => <div className="flex flex-wrap gap-2">
      ...
      {canChangeTier ? <ChangeMemberTierDialog currentMembershipId={member.membershipId} memberId={member.id} memberName={`${member.firstName} ${member.lastName}`} onChanged={() => refreshAfterMutation(`${member.firstName} ${member.lastName}'s tier was changed.`)} /> : null}
      {canSuspend && member.isActive ? <AdminJustificationDialog confirmLabel="Suspend" description={`Suspend ${member.firstName} ${member.lastName}'s customer and login access while retaining their membership record.`} onConfirm={(justification) => suspendMember(member, justification)} title="Suspend member" triggerLabel="Suspend" variant="danger" /> : null}
    </div> },
  ];

  return <main className="min-h-dvh px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-6">
    <header><Breadcrumb items={[{ href: "/admin/dashboard", label: "Admin" }, { label: "Members" }]} /><h1 className="mt-2 text-3xl font-bold">Members</h1><p className="mt-2 text-muted-foreground">Maintain member profiles, membership tiers, and account access.</p></header>
    <Card><div className="flex items-center gap-3"><UsersRound className="size-5 text-accent" /><div><p className="text-sm text-muted-foreground">Members</p><p className="text-2xl font-bold">{members.length}</p></div></div></Card>
    {error ? <StatusMessage tone="error">{error}</StatusMessage> : null}
    {loading ? <Skeleton className="h-72" /> : <DataTable columns={columns} data={members} emptyState="No members found." keyExtractor={(member) => member.id} searchFn={(member, query) => `${member.firstName} ${member.lastName} ${member.email} ${member.membershipName}`.toLowerCase().includes(query)} />}
  </div></main>;
};
