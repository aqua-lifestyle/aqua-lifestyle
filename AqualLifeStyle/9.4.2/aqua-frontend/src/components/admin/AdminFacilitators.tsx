...
export const AdminFacilitators = () => {
  ...
  const mutateFacilitator = async (operation: FacilitatorMutation, facilitator: AdminFacilitator, justification: string) => {
    ...
    toast({ message: `${facilitator.customerName}: ${operation.toLowerCase()} completed.`, title: "Admin action completed", type: "success" });
  ...
  const columns = [
    ...
    { header: "Area leader", key: "areaLeaderId", sortable: true, render: (facilitator: AdminFacilitator) => `#${facilitator.areaLeaderId}` },
    ...
    { header: "Actions", key: "actions", render: (facilitator: AdminFacilitator) => <div className="flex flex-wrap gap-2">
      {!facilitator.isApproved && can("Approve") ? <AdminJustificationDialog confirmLabel="Approve" description={`Approve ${facilitator.customerName} and assign the Facilitator identity role.`} onConfirm={(reason) => mutateFacilitator("Approve", facilitator, reason)} title="Approve facilitator" triggerLabel="Approve" /> : null}
  ...
  return <main className="min-h-dvh px-4 py-6 sm:px-6 lg:px-8"><div className="mx-auto flex max-w-7xl flex-col gap-6">
    <header><Breadcrumb items={[{ href: "/admin/dashboard", label: "Admin" }, { label: "Facilitators" }]} /><h1 className="mt-2 text-3xl font-bold">Facilitators</h1><p className="mt-2 text-muted-foreground">Review applications, control rank changes, and track referral awards.</p></header>
