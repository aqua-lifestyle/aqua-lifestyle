...
namespace AqualLifeStyle.EntityFrameworkCore
{
        ...
        : AbpZeroDbContext<Tenant, Role, User, AqualLifeStyleDbContext>, IDataProtectionKeyContext
    {
        ...
        public virtual DbSet<AQGreenFuneralCoverEntitlement> AQGreenFuneralCoverEntitlements { get; set; }
        ...
        private void EnsureAQGreenRecruitmentAttributionIsAppendOnly()
        {
            if (ChangeTracker.Entries<AQGreenRecruitmentAttribution>().Any(entry =>
                    entry.State == EntityState.Modified ||
                    entry.State == EntityState.Deleted) ||
                ChangeTracker.Entries<AQGreenRecruitmentAttributionConfirmation>().Any(entry =>
                    entry.State == EntityState.Modified ||
                    entry.State == EntityState.Deleted))
            {
                throw new InvalidOperationException(
