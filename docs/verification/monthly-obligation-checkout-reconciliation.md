...
|---|-----------|-------|---------------|
| 1 | `YocoPaymentNotificationProcessor.ProcessAsync` → `ProcessAQGreenMonthlyObligationCheckoutAsync` (monthly-payment branch) | `src/AqualLifeStyle.Application/Payments/YocoPaymentNotificationProcessor.cs` | The checkout exists in `AwaitingPayment`; the merchant reference identifies the checkout; amount/currency/purpose/merchant facts verify; but the target obligation is not in the exact state the settlement requires (not unpaid, missing, wrong period, or already settled). |
...
The verification method `EnsureNotificationMatchesCheckout` (join checkout + obligation + customer facts, merchant-reference match, amount/currency match, monthly-purpose check, obligation-period match) is executed for every monthly event before any state change, and again in the replay path.
...
### B1. Verified monthly webhook for an obligation that is no longer in "fully unpaid" state (EVIDENCE-COMPLETE BUT INVARIANT-BLOCKED — AUTHORISED ACTION REQUIRED)
`ProcessAQGreenMonthlyObligationCheckoutAsync` settles only when the obligation is still unpaid and matches the checkout period. When settlement succeeds, the obligation becomes `Paid`, `PaymentId` is set, and commission for the period becomes payable *after* verified payment. When a verified payment cannot settle (e.g. the obligation was settled by a different verified payment, or an overdue state was assessed before settlement), the processor records `ReconciliationRequired` with evidence and **does not classify commission**. The payment itself is a confirmed, verified `MemberPayment` — evidence is complete; no money is unaccounted for.
...
2. **Read-only reconciliation discovery/triage queue** — `IAdminProgrammeParticipationAppService.GetMonthlyObligationCheckoutReconciliationAsync`:
   - Scope: host-wide requires `Admin.AllTenants`; tenant sessions see only their own Area; `ValidateRequestedTenant` rejects cross-Area requests; same permission gate as the legacy reconciliation screen (`Admin.ProgrammeParticipations.ViewLegacyPaymentReconciliation`).
   - Filters: `TenantId`, `PeriodYear`, `PeriodMonth`; paged; ordered by completion then creation (newest first).
   - Rows: checkout id, Area, club number, customer name, period, amount, currency, status, provider checkout id, payment id + provider reference, allocation status, allocation evidence, created/completed times, `IsPaymentAllocated` (checkout payment == obligation payment), and the recorded obligation status.
   - Read-only: no repository writes; uses `DisableAllTenantDataFiltersForHost` exactly like sibling admin endpoints; no data returned beyond the operator checklist (Part F visibility: reason, detected-at, payment reference, amount/currency, recorded obligation state).
...
| 2 | `WeeklyCommissionCalculator` computes AQGreen holds from current obligation/loan state (§17.10); a post-cutoff overdue assessment holds the previous cycle's commission | pre-existing documented gap; blocks arming | Accepted debt; worker remains disabled (`App:WeeklyCommissions.Enabled=false`). |
