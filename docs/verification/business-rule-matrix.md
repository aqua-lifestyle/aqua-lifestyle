...
| M-15 | Automatic obligation scheduling and payment allocation into obligations | **Confirmed, implemented (gap-closure)** | AT/I | `EntryMonthlyObligationScheduler` + `ConfigurationEntryMonthlyObligationDueDatePolicy` (1–28, UTC midnight; null when unset), config-gated `EntryMonthlyObligationWorker`, advisory-lock `EntryMonthlyObligationSchedulingLock` (PG `0x415147524F424C`), idempotent assessment/allocation; `EntryMonthlyObligationSchedulerTests` (5) + `ConfigurationEntryMonthlyObligationDueDatePolicyTests` (7). BR-16 confirms R600 monthly is separate from R1,200 joining. `onyx-implementation-plan.md:189-190` defers scheduling to the secured workflow phase — the worker is disabled by default (`App:EntryMonthlyObligations:Enabled=false`) and never invents due dates (PD-07 remains unresolved) |
