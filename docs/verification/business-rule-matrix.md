...
| M-15 | Automatic obligation scheduling and payment application | **Scheduling implemented; payment workflow incomplete** | AT/I | `EntryMonthlyObligationScheduler` + durable host-level `EntryMonthlyObligationDuePolicy` versions implement fail-closed scheduling/assessment. Policies use day 1–28 and an effective AQGreen obligation month at 00:00 `Africa/Johannesburg` stored as UTC. The first obligation month is after activation, and each new obligation records the selected policy version. The table starts empty and the worker remains disabled. Payment checkout must persist one authoritative `ObligationId` and period; the rejected oldest-open heuristic was removed and unlinked confirmations require reconciliation. |
...
| M-28 | No member-facing commission ledger is exposed | **Confirmed, implemented (gap-closure)** | AT/I | `ClubMemberProgrammeProgressAppService.GetMyProgressAsync` (member-facing, `Aqua.ProgrammeParticipations.ViewSelf`) exposes current qualified level, direct-recruit progress, weekly earning components with payout status/hold reason, and monthly obligation status; `ClubMemberProgrammeProgressAppServiceTests` (4). No member mutation path exists — read-only transparency view only |
...
| M-44 | Member-visible education component about the programme/tree/commissions | **Confirmed, implemented (gap-closure)** | AT/I | `ClubMemberProgrammeProgressAppService` returns static education items (R1,200 joining → R30,000 funeral cover; 5 direct recruits per level; weekly components 150/250/1250 ZAR; R600 monthly with grace); rendered by `member-programme-progress.tsx`; `MemberProgrammeProgress` tests assert all four items |
| M-45 | Member-facing commission ledger / transparency view | **Confirmed, implemented (gap-closure)** | AT/I | `ClubMemberProgrammeProgressAppService.GetMyProgressAsync` exposes qualified level, direct-recruit progress, per-week earning components with payout status/hold reason, monthly obligation status/next action, and funeral-cover inclusion; gated by `Aqua.ProgrammeParticipations.ViewSelf`; `ClubMemberProgrammeProgressAppServiceTests` (4) cover earned/held/paid and inactive-customer rejection |

## 11. Funeral cover

| ID | Rule | Status | Evidence | Source |
|----|------|--------|----------|--------|
| M-46 | R30,000 funeral cover plan with 6-month waiting period, external insurer | **Confirmed, implemented (gap-closure)** | AT/I | FR-44 `requirements.md:86`; BR-10 `requirements.md:130` (6-month waiting); A-21 `Assumptions.md:40` (external insurer; platform tracks linkage/eligibility only). Implemented: `AQGreenFuneralCoverInclusionProcessor.EnsureIncludedAsync` records an `Included` `AQGreenFuneralCoverEntitlement` (version `2026-08-funeral-30000`, R30,000, ZAR) once `EntryParticipation.IsJoiningObligationSatisfied` (R1,200 once or R600+R600); wired into both full and two-installment paths of `ProgrammePaymentConfirmationProcessor`; migration `20260807065821_AddAQGreenFuneralCoverEntitlements`; `AQGreenFuneralCoverTests` (6 + 2 processor + 1 two-installment). Insurance activation/enrolment timing and the 6-month waiting period remain separate (PD-06) |
