...
4. **Frontend**: no env change needed for the new page; the navbar link appears automatically for members with `ViewSelf`.
5. **Logs**: structured `ProgrammeEngineAlert` events for processed obligations and any undefined-due-date condition; no secrets/customer/payment data logged.
