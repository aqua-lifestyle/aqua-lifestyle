...
| Member payment and Yoco reconciliation ledger | 🔧 Workflow in progress | Direct Onyx uses Yoco hosted checkout, a three-minute replay window, constant-time HMAC-SHA256 webhook verification, exact amount/currency/mode matching, provider-reference uniqueness, and idempotent transactional activation. AQGreen and other payment-purpose checkout adapters remain |
