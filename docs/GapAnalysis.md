...
| AQGreen feeder participation | 🔧 Workflow in progress | Secured self-service joining and status, independent or verified optional-recruiter placement, one confirmed R1,200 Yoco joining payment, permanent history, and read-only administrator reconciliation are implemented; automated monthly-payment allocation remains |
| Direct Onyx participation | ✅ Implemented | Secured self-service joining creates only a stable Yoco checkout intent. A signature-verified, amount-matched R6,120 webhook atomically creates the active Onyx participation and intended network placement, grants Club Member access without downgrading higher roles, and does not create AQGreen history. Cancelled, failed, or abandoned checkout creates no participation |
...
| Member payment and Yoco reconciliation ledger | 🔧 Workflow in progress | Direct Onyx and AQGreen joining use Yoco hosted checkout, a three-minute replay window, constant-time HMAC-SHA256 webhook verification, exact checkout/amount/currency/mode matching, provider-reference uniqueness, and idempotent transactional activation. Operational reconciliation, refunds, disputes, and other payment-purpose adapters remain |
