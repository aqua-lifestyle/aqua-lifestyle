...
| Entry feeder participation | 🔧 Foundation implemented | Supports independent joining or an optional verified Entry recruiter, two confirmed R600 activation payments, versioned terms, permanent history, and PostgreSQL persistence; secured member/admin workflows remain |
...
| Savings accounts with monthly deposits (current min R100; older account thresholds unresolved) | ⚠️ Partial | The domain-only `SavingsAccount` is not persisted and does not enforce R100. Its first-year lock currently blocks deposits rather than withdrawals, so it must be corrected before persistence |
| Savings window rules (deposits 1st–15th; locked 17th–24th admin period) | ⚠️ Partial | `SavingsAccount` enforces deposits from the 1st–15th, but the administration-period and withdrawal-lock semantics are not represented correctly |
